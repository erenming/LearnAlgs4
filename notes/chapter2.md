...
2. 排序算法分析:
    - 最好、最坏、平均情况时间复杂度
    - 时间复杂度的系数、常数、低阶：对于规模较小时要考虑
    - 比较次数和交换（复制）次数
2. 排序算法稳定性：
    - 对于待排序序列中的相等元素，经过排序后，其先后顺序不变，则为稳定的排序算法
...
5. 归并排序：将两个有序数组归并成一个更大的有序数组
    - 递推公式：`merge_sort(q...r) = merge(merge_sort(p...q), merge_sort(p+1...r))`
    - 归并方法：
...
6. 快速排序：分治思想
    ...
    - 时间复杂度：
        - 大部分: O(nlogn)
        - 极端：O(n^2)
7. 线性排序：时间复杂度为线性O(n)，但是对数据要求苛刻
    - 桶排序
        1. 将待排序的数据分到几个有序的桶里，每个桶里的数据单独排序
        2. 排完后，按桶的顺序依次取出数据，组成后即为有序数据了
        3. 适用场景：比较适合在外部排序中
            - 例如，外部数据在磁盘中，数据量大而内存不足时
    - 计数排序：桶排序的特殊情况
        1. 当要排序n个数据，且所处范围不大，比如最大值为k, 则把数据划分为k个桶，每个桶的数据都等值
        2. 按桶顺序依次取出即可
        3. 使用场景：当要排序n个数据，且所处范围不大
            - 例如，以成绩排序50万考生
    - 基数排序
        1. 对数据按位分割，从低向高位分别排序，由于排序的稳定性，故可实现O(n)的时间复杂度
        2. 适用场景：
            - 基数排序对要排序的数据是有要求的，需要可以分割出独立的“位”来比较，而且位之间有递进关系
            - 每一位的数据不能太大，要可以用线性排序来排序
8. 排序的应用
    - 问题的规约:
        1. 找出重复的元素
        2. 排名问题
